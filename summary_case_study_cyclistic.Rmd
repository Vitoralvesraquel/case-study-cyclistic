...
```{r trip_percent_weekday fig.width=14}
hour_df <- df %>% 
...
```{r trip_percent_month fig.width=14}
month_df <- df %>% 
...
```{r trip_count_hour_week fig.width=14}
ggplot(data = df) +
...
```{r trip_percent_hour_week ig.width=14}
ggplot(data = hour_df) +
    geom_bar(position = position_dodge(), stat = "identity") +
    facet_wrap("day_of_week") +
    scale_x_continuous(breaks = seq(0, 24, 3),
...
```{r trip_percent_day_month fig.width=14}
day_df <- df %>% 
